return {
  ...
  config = function()
    ...
    local function extract_neogit_filepath(line)
      -- Typical status lines look like:
      -- "> modified path/to/file.lua"
      -- "> new file path/to/file.lua"
      -- "> deleted path/to/file.lua"
      --
      -- Grab everything after the status word(s).
      local patterns = {
        "^%s*[>v]?%s*modified%s+(.+)$",
        "^%s*[>v]?%s*new file%s+(.+)$",
        "^%s*[>v]?%s*deleted%s+(.+)$",
